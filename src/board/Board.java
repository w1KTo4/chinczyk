...
public class Board {
    ...
    public boolean attemptMove(int fromRow, int fromCol, int toRow, int toCol, int newCounter) {
        Piece moving = getPieceAt(fromRow, fromCol);
        if (moving == null) return false;
        if (!valid(toRow, toCol)) return false;
        Piece target = getPieceAt(toRow, toCol);
        if (target != null) {
            if (target.getColor() == moving.getColor()) {
                return false;
            }
            removePieceAt(toRow, toCol);
            sendPieceToBase(target);
        }
        removePieceAt(fromRow, fromCol);
        moving.setCounter(newCounter);
        setPieceAt(toRow, toCol, moving);
        return true;
    }
    private void sendPieceToBase(Piece piece) {
        piece.setCounter(0);
        Point base = getFreeBasePosition(piece.getColor());
        setPieceAt(base.y, base.x, piece);
    }
    private Point getFreeBasePosition(PieceColor color) {
        int[][] basePositions;
        switch (color) {
            case RED:
                basePositions = new int[][] { {2,2}, {3,2}, {2,3}, {3,3} };
                break;
            case GREEN:
                basePositions = new int[][] { {11,2}, {11,3}, {12,2}, {12,3} };
                break;
            case BLUE:
                basePositions = new int[][] { {2,11}, {3,11}, {2,12}, {3,12} };
                break;
            case YELLOW:
            default:
                basePositions = new int[][] { {11,11}, {12,11}, {11,12}, {12,12} };
                break;
        }
        ...
        return new Point(basePositions[0][0], basePositions[0][1]);
    }
